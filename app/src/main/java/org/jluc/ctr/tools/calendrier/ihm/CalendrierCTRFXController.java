...
public class CalendrierCTRFXController {
    ...
    public void init(CalendrierCTRController controller) {
        ...
        mApplyFilterBtn.setOnAction(event -> {
            filteredEvents.setPredicate(evenement -> {
                ...
                boolean isDemandeurOK = mFiltreDemandeurCmb.getValue().getName().equals("Tous")
                        || evenement.getDemandeur().equals(mFiltreDemandeurCmb.getValue());
                boolean isActiviteOK = mFiltreActiviteCmb.getValue().equals(TypeActivite.ALL)
                        || evenement.getType().getActivite().equals(mFiltreActiviteCmb.getValue());
    ...
    private void validateEvent(Evenement eventSelected) {
        try {
            ...
            Alert alert = new Alert(AlertType.INFORMATION, "Ajout de l'évènement dans le calendrier CTR",
                    ButtonType.OK);
            alert.showAndWait();

        } catch (IOException | EmailException e) {
            mLogger.error("Probleme durant l'ajout de l'evenement dans le calendrier CTR", e);
            Alert alert = new Alert(AlertType.ERROR, "Problème durant l'ajout de l'évènement dans le calendrier CTR",
                    ButtonType.OK);
            alert.showAndWait();
        }
    }
    ...
    private void modifiyEvenement(Evenement eventSelected) {
        eventSelected
                .setDateDebut(Date.from(mDateDebutPicker.getValue().atStartOfDay(ZoneId.systemDefault()).toInstant()));
 ...
 */
    private void fillEventEditor(Calendrier model, Evenement eventSelected) {
        ...
        FilteredList<Evenement> filteredEvents = new FilteredList<>(data, evenement -> {
            Date today = new Date();
            boolean isDateOK = evenement.getDateDebut().after(today);
            boolean isConflitOK = ((evenement.getDateDebut().before(eventSelected.getDateFin())
                    || evenement.getDateDebut().equals(eventSelected.getDateFin()))
                    && (evenement.getDateFin().after(eventSelected.getDateFin())
                            || evenement.getDateFin().equals(eventSelected.getDateFin())))
                    || (evenement.getDateDebut().before(eventSelected.getDateDebut())
                            && evenement.getDateFin().after(eventSelected.getDateDebut()))
                    || (evenement.getDateDebut().after(eventSelected.getDateDebut())
                            && evenement.getDateFin().before(eventSelected.getDateFin()));

            return isDateOK && isConflitOK && !evenement.equals(eventSelected);
        });
        SortedList<Evenement> sortedEvents = new SortedList<>(filteredEvents);
